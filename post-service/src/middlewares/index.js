...
import Joi from "joi";
import { UnauthorizedAPIError, NotFoundAPIError, BadRequestAPIError } from "../errors/index.js";
...
class Middlewares {
   ...
   * @returns {import('express').Response}
   */
  static async checkPostIdParam(request, response, next) {
    const { id: postId } = request.params;

    if (!mongoose.Types.ObjectId.isValid(postId)) {
      logger.warn(`Invalid post id: ${postId}`);
      throw new BadRequestAPIError("Invalid post id");
    }
    next();
  }
}
